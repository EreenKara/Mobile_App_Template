...
const GroupScreen: React.FC<GroupProps> = ({ navigation, route }) => {
   ...
   const renderRightActions = (userId: string) => {
      return (
         <TouchableOpacity style={styles.deleteButton} onPress={() => handleDelete(userId)}>
            <Image source={require('@assets/images/trash_can.png')} style={styles.deleteIcon} />
         </TouchableOpacity>
      );
   };
      ...
      return (
         <Swipeable renderRightActions={() => renderRightActions(item.id || '')}>
            <View style={styles.userItem}>
               <MenuItemComponent
                  touchable={false}
                  icon={item.image ? item.image : require('@assets/images/no-avatar.png')}
   ...
   if (error) {
      return <ErrorScreenComponent fromScreen="Group" toScreen="Groups" error={error} />;
   }
